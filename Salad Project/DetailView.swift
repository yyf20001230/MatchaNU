        ...
        if !self.classes.detail.isEmpty{
            VStack {
                ...
                HStack{
                    ...
                    if !(self.classes.userClass.contains(self.classes.detail[0])){
                        Button("ADD"){
                        ...
                        .alert(isPresented: self.$showAlert){
                            Alert(title: Text("Class Dropped"), message: Text("\(classes.detail[0].Major.components(separatedBy: " ")[0] + " " + classes.detail[0].Class.components(separatedBy: " ")[0]) is dropped"), dismissButton: .default(Text("Got it!")))
                        }
                        .padding(.all)
                        ...
                        Button("DROP"){
                            self.classes.userClass = self.classes.userClass.filter{$0 != self.classes.detail[0]}
                            self.showAlert = true
                        }
                        .alert(isPresented: self.$showAlert){
                            Alert(title: Text("Class Added"), message: Text("\(classes.detail[0].Major.components(separatedBy: " ")[0] + " " + classes.detail[0].Class.components(separatedBy: " ")[0]) is added to your schedule"), dismissButton: .default(Text("Got it!")))
                        }
                        .padding(.all)
                        .accentColor(Color(#colorLiteral(red: 0.9176470588, green: 0.3450980392, blue: 0.3019607843, alpha: 1)))
                        .background(Color(#colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)))
                        .clipShape(RoundedRectangle(cornerRadius: 5))
                    }
